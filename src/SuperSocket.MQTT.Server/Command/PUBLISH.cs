...
namespace SuperSocket.MQTT.Packets
{
    ...
    public class PUBLISH : IAsyncCommand<MQTTPacket>
    {
        ...
        public async ValueTask ExecuteAsync(IAppSession session, MQTTPacket package)
        {
            ...
            var b = sessions.Where(x => x.TopicNames.Any(x => IsMatch(pubpacket.TopicName,x.TopicName)));//;x.TopicName.Equals(pubpacket.TopicName, StringComparison.OrdinalIgnoreCase)));
            await Task.Run(() =>
            {
